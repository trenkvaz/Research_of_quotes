...
public class PrepareDataML {
    ...
    public static void startPrepare(String startDay,String endDay){
        ...
        JSONArray partDays = alignmentHoursInDay(getArrayDays(startDay,endDay));
       ...
       cutFinalMin5Sec(cutPeriods,7,partDaysMin5Sec);
    }
    // [
    ...
    static List<JSONObject> cutFinalMin5Sec(JSONArray periodsDays,int sizeDays,JSONObject partDaysMin5Sec){
        ...
        List<Integer> resultTest = new ArrayList<>(100000000);
        for(int i=0; i< 1; i++){
            ...
            for (int d=0; d<sizeFirstDaysCandle; d++){
                ...
                for(int cD=0; cD<4; cD++) firstDaysCandlesAndDayHours[cD+(d*4)] = candleDay[cD];
            }
            ...
            for (int h=0; h<sizeDayHoursCandle; h++){
                ...
                for(int cD=0; cD<4; cD++) firstDaysCandlesAndDayHours[20+cD+(h*4)] = candleHour[cD];
            }
            ...
            for(int m=10; m<sizeMinutesCandle; m++){
                ...
                for (int m1=0; m1<m; m1++){
                    ...
                    for(int cD=0; cD<4; cD++) minutes[cD+m1*5] = candleMinute[cD];
                    minutes[4+m1*5] = minute.getInt("volumeMinute");
                }
                ...
                for (int s=0; s<size5SecCandle; s++){
                    ...
                    for(int cD=0; cD<5; cD++) sec5[cD+s*5] = candle5sec[cD];
                    Integer[] dataML = new Integer[720];

                    for(int add=0; add<firstDaysCandlesAndDayHours.length; add++)dataML[add] = firstDaysCandlesAndDayHours[add];
                    for(int add=0; add<minutes.length; add++)dataML[add] = minutes[add];
                    for(int add=0; add<sec5.length; add++)dataML[add] = sec5[add];
